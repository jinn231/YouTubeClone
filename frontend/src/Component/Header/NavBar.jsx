...
const NavBar = () => {
  ...
  const DynamicMenuContainer = Style.div`
  position: fixed;
  z-index: 100;
  background: white;
  width: 270px;
  ...
  const SignInWithGoogle = async () => {
    try {
      ...
      const resp = await Axios.post("/auth/google", data)
  ...
  return (
    <>
        <div className="flex z-10 justify-between fixed w-full top-0 bg-white border pb-3">
            <div className="flex mx-2 gap-1 md:mx-6 md:gap-6" >
              <div className="mt-3" onClick={handleNav}>
            ...
            <div className="mt-2.5">
              <div className="md:flex hidden">
                <input id="search__input" className="w-96 h-11 px-5 font-sans outline-none border p-2" placeholder="Search" type="text"  />
                <button id="search_input_btn" className="border h-11 p-2 pl-6 bg-slate-100 pr-4">
                  <SearchOutlinedIcon />
                </button>
              </div>
          ...
          {
            currentUser ? (
              <div className="mt-2.5 mx-3 md:mx-6 relative">
                <ul className="flex justify-between gap-1 md:gap-3">
                  <li className="md:hidden p-2 rounded-full hover:bg-gray-200 cursor-pointer">
                    <SearchOutlinedIcon  fontSize="medium" />
                  </li>
                  <li className="p-2 rounded-full hover:bg-gray-200 cursor-pointer">
                    <Link to={`/channel/${currentUser._id}#uploads`}>
                      <VideoCallOutlinedIcon  fontSize="medium" />
                    </Link>
                  </li>
                  <li className="p-2 rounded-full hidden md:inline hover:bg-gray-200 cursor-pointer">
                    <NotificationsOutlinedIcon fontSize="medium" />
                  </li>
                  <li className="bg-gray-200 rounded-full cursor-pointer">
                    <Link to={`/channel/${currentUser._id}`}>
                      <img className="w-10 h-10 rounded-full" src={currentUser.image} alt="" />
                    </Link>
                  </li>
                  <li className="p-2">
                    <Link to={`/channel/${currentUser._id}`}>
              ...
              <div className="m-1 mt-2 md:mx-6">
                  <button onClick={SignInWithGoogle} className="py-2 px-3 rounded-full shadow hover:bg-blue-100 text-blue-600 border text-sm flex flex-1 font-robo"><PermIdentityOutlinedIcon className="mx-1" fontSize="medium"/><p className="mt-0.5 font-bold">Sign In</p></button>
              </div>
            )
          }
        </div>
